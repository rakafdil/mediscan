...
import { faPrint, faArrowLeft, faPlus } from '@fortawesome/free-solid-svg-icons'
...
const HistoryPageDetail = ({ user }: { user: User | null }) => {
    ...
    const handleRetake = (sessionId: string | number) => {
        if (confirm("Do you want to retake the diagnosis with the same symptoms?")) {
            fetch(`/api/history/${sessionId}/retake`, {
                method: "POST",
                ...
                <div className="bg-gray-50 border-b border-gray-200 p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-5 text-2xl">
                            <button onClick={() => window.print()} className="hover:text-blue-600 cursor-pointer">
                                ...
                                <ul className="space-y-3">
                                    {scanDetail?.scan_history_symptoms?.map((s, index) => (
                                        <li key={index} className="flex items-start">
                                            <span className="h-5 w-5 rounded-full bg-blue-500 flex items-center justify-center mt-1">
                                                ✔
                                            </span>
